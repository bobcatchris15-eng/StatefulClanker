...
'evil'{if([string]::IsNullOrWhiteSpace($Subcommand)){$Subcommand='status'};switch($Subcommand.ToLowerInvariant()){'clear'{Clear-SCEvilTrip $(if($Reason){$Reason}else{'Cleared explicitly by operator.'});Write-Host 'Evil safety trip cleared.';break};'status'{$e=if(Test-SCEvilLatched){Read-SCJson (Get-SCEvilPath)}else{$null};if($e){Write-Host "EVIL LATCHED since $($e.firstAt): $($e.reason) [$($e.trigger)]";Write-Host "Trip count: $($e.tripCount)  Last trip: $($e.lastAt)"}else{Write-Host 'Not evil.'};break};default{throw "Unknown evil subcommand: $Subcommand"}};break}
