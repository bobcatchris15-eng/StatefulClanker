...
try {
    ...
    $catalog=[ordered]@{
      schemaVersion=1
      allow=@('builtin.*','intent.human.read','intent.normalized.read','mcp.toaster.search','mcp.toaster.read')
      deny=@('builtin.run_command')
      sources=[ordered]@{
        toaster=[ordered]@{
          transport='http';url='http://127.0.0.1:9999/mcp';enabled=$true
          tools=@(
    ...
    $external=@(Get-SCExternalWorkerToolRecords $worker 'worker')
    Assert-True ($external.Count-eq2) 'Only the two machine-authorized Toaster tools should be advertised.'
    Assert-True (@($external.capability)-contains'mcp.toaster.search') 'Toaster search missing from advertised records.'
    Assert-True (@($external.capability)-notcontains'mcp.toaster.write_lesson') 'Unauthorized write_lesson was advertised.'

    $registry=@(Get-SCWorkerToolRecords $worker 'worker')
    $human=$registry|Where-Object wireName -eq'read_human_intent';$normalized=$registry|Where-Object wireName -eq'read_normalized_intent'
    Assert-True ($null-ne$human) 'Human intent reader was not advertised.'
    Assert-True ($null-ne$normalized) 'Normalized intent reader was not advertised.'
    $humanResult=Invoke-SCWorkerTool 'read_human_intent' ([pscustomobject]@{sourceRef='human:h-test'}) $worker 'worker' $registry
    Assert-True ($humanResult -match 'verbatim human intent') 'Human reader did not return direct source evidence.'
    $normalizedResult=Invoke-SCWorkerTool 'read_normalized_intent' ([pscustomobject]@{}) $worker 'worker' $registry
    Assert-True ($normalizedResult -match 'normalized objective' -and $normalizedResult -match 'use bounded workers') 'Normalized reader did not return Intent plus current directives.'

    Write-Host 'PASS: worker capability policy is tighten-only at runtime and exposes separate read-only intent views.'
} finally {
