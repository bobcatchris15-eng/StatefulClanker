...
"; q.ExecuteNonQuery();
    ...
    static int Query(string project,string text,string paths,int limit)
    {
        ...
        foreach(var t in terms){var p="$t"+i++;where.Add($"(lower(l.title) LIKE {p} OR lower(l.body) LIKE {p} OR EXISTS(SELECT 1 FROM lesson_tags lt WHERE lt.lesson_id=l.id AND lower(lt.tag) LIKE {p}))");q.Parameters.AddWithValue(p,"%"+t+"%");}
