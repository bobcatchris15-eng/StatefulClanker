...
sealed class WorkerCapabilitiesPage : TabPage
{
    ...
    void EditProfile(string? name)
    {
        ...
        if (name is not null && !string.Equals(name, d.ProfileName, StringComparison.OrdinalIgnoreCase)) profiles.Remove(name); profiles[d.ProfileName] = new JsonObject { ["allow"] = string.IsNullOrWhiteSpace(d.Allow) ? null : ToArray(d.Allow), ["deny"] = ToArray(d.Deny) }; WorkerCapabilityStore.Save(_root); Reload();
    }
    ...
    void EditSource(string? name)
    {
        ...
        if (name is not null && !string.Equals(name, d.SourceName, StringComparison.OrdinalIgnoreCase)) RemoveSource(name); sources = (_root["sources"] as JsonObject)!; sources[d.SourceName] = d.Source; foreach (var pattern in Split(d.AllowPatterns)) AddUnique(_root["allow"]!.AsArray(), pattern); WorkerCapabilityStore.Save(_root); Reload();
    }
...
{
    readonly TextBox _allow = new(), _deny = new(); public string Allow => _allow.Text; public string Deny => _deny.Text;
    public AllowDenyDialog(string title, string allow, string deny)
    {
        Text = title; Width = 650; Height = 360; StartPosition = FormStartPosition.CenterParent; var t = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 1, RowCount = 5, Padding = new Padding(12) }; t.RowStyles.Add(new RowStyle(SizeType.Absolute, 24)); t.RowStyles.Add(new RowStyle(SizeType.Percent, 50)); t.RowStyles.Add(new RowStyle(SizeType.Absolute, 24)); t.RowStyles.Add(new RowStyle(SizeType.Percent, 50)); t.RowStyles.Add(new RowStyle(SizeType.Absolute, 42)); _allow.Multiline = _deny.Multiline = true; _allow.Text = allow; _deny.Text = deny; t.Controls.Add(new Label { Text = "ALLOW patterns (comma/line separated)", Dock = DockStyle.Fill }, 0, 0); t.Controls.Add(_allow, 0, 1); t.Controls.Add(new Label { Text = "DENY patterns (deny wins)", Dock = DockStyle.Fill }, 0, 2); t.Controls.Add(_deny, 0, 3); var bar = new FlowLayoutPanel { Dock = DockStyle.Fill, FlowDirection = FlowDirection.RightToLeft }; bar.Controls.Add(new Button { Text = "Save", DialogResult = DialogResult.OK, Width = 90 }); bar.Controls.Add(new Button { Text = "Cancel", DialogResult = DialogResult.Cancel, Width = 90 }); t.Controls.Add(bar, 0, 4); Controls.Add(t); Theme.Apply(this);
    }
}
sealed class ProfileDialog : AllowDenyDialog
{
    readonly TextBox _name = new(); public string ProfileName => _name.Text.Trim();
    public ProfileDialog(string? name, string allow, string deny) : base("Capability profile", allow, deny) { _name.Text = name ?? ""; var prompt = Microsoft.VisualBasic.Interaction.InputBox("Profile name", "Capability profile", _name.Text); if (!string.IsNullOrWhiteSpace(prompt)) _name.Text = prompt.Trim(); }
}
sealed class SourceDialog : Form
{
    ...
    {
        if (string.IsNullOrWhiteSpace(SourceName) || string.IsNullOrWhiteSpace(_url.Text)) { MessageBox.Show(this, "Source name and URL are required."); DialogResult = DialogResult.None; return; }
