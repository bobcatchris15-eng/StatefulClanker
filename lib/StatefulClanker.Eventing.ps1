...
function Get-SCControlEventLevel([string]$Type) {
    ...
    if($Type -match '^(intent\.revised|directive\.(revised|retired)|plan\.imported|task\.(completed|complete)|project\.review)'){return 'attention'}
    return 'fyi'
}

function Publish-SCControlEvent([string]$Type,[string]$Text,$Data=$null,[string]$Level=$null) {
    Ensure-SCControlEventLayout
    if([string]::IsNullOrWhiteSpace($Level)){$Level=Get-SCControlEventLevel $Type}
    if(@('fyi','attention','human_required') -notcontains $Level){throw "Invalid control event level '$Level'."}
    $record=$null
    Invoke-SCLocked {
        $statePath=Get-SCPath 'control/state.json'
        $controlState=Read-SCJson $statePath
        $last=0;if($controlState-and$controlState.PSObject.Properties['lastSequence']){$last=[long]$controlState.lastSequence}
        $sequence=$last+1
        $record=[ordered]@{schemaVersion=1;sequence=$sequence;id=New-SCId 'control';ts=(Get-Date).ToUniversalTime().ToString('o');level=$Level;type=$Type;message=$Text;data=$Data}
        ((ConvertTo-SCJson $record 14)-replace"`r?`n",'')|Add-Content -LiteralPath (Get-SCPath 'control/events.jsonl') -Encoding UTF8
        Write-SCJson $statePath ([ordered]@{schemaVersion=1;lastSequence=$sequence;updatedAt=$record.ts})
    }|Out-Null
    return $record
}
...
function Get-SCControlCursor {
    Ensure-SCControlEventLayout
    $state=Read-SCJson (Get-SCPath 'control/state.json')
    if($null-eq$state-or-not$state.PSObject.Properties['lastSequence']){return 0}
    return [long]$state.lastSequence
}

function Get-SCControlEventsSince([long]$Since=0,[int]$Limit=100,[string]$MinimumLevel=$null) {
    Ensure-SCControlEventLayout
    $limitValue=[Math]::Min(1000,[Math]::Max(1,$Limit))
    ...
    foreach($line in @(Get-Content -LiteralPath (Get-SCPath 'control/events.jsonl')|Where-Object{$_})) {
        ...
        $out+=,$evt
        if($out.Count-ge$limitValue){break}
    }
    return @($out)
}
