...
function New-SCWorkerPrompt($Compilation) {
    return "You are a cold-start StatefulClanker worker. The compiled packet is a temporary projection; durable state and project files are authoritative. The project intent contract inside the packet is orchestrator-owned and READ-ONLY: never modify or weaken it. Raise INTENT_QUESTION or INTENT_CONFLICT instead of guessing or changing intent.`r`n`r`nSTATEFULCLANKER COMPILED CONTEXT`r`n================================`r`n$(ConvertTo-SCJson $Compilation.ir 22)`r`n`r`nComplete only this bounded task."
}
