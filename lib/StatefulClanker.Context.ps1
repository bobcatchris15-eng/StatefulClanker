...
function New-SCCompilation($Task) {
    ...
    $ir=[ordered]@{schemaVersion=2;compilationId=$compilationId;compiledAt=(Get-Date).ToUniversalTime().ToString('o');project=[ordered]@{goal=$state.goal;root=Get-SCRoot;activePlan=$planIntent;directionRevision=$state.directionRevision;stateRevision=$state.revision;executionPolicyHash=$policyHash;intent=[ordered]@{revision=[int]$intent.revision;hash=$intentHash;authority='orchestrator-owned; worker read-only';contract=$intent}};task=[ordered]@{id=$Task.id;title=$Task.title;instruction=$Task.instruction;role=$taskRole;outputKind=if($Task.PSObject.Properties['outputKind']){[string]$Task.outputKind}else{'change'};controlRevision=$taskControlRevision;acceptance=@($Task.acceptance);checks=if($Task.PSObject.Properties['checks']){@($Task.checks)}else{@()};semanticAcceptance=if($Task.PSObject.Properties['semanticAcceptance']){@($Task.semanticAcceptance)}else{@()};dependsOn=@($Task.dependsOn);relations=if($Task.PSObject.Properties['relations']){@($Task.relations)}else{@()};latestFeedback=$latestFeedback};dependencies=$dependencies;sources=[ordered]@{retrieved=$retrieved;recentEvents=@(Get-SCRecentEvents $eventCount $eventBudget)};outputContract=$contract}
