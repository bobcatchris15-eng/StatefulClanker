...
# concrete attempt to leave the worker root, mutate StatefulClanker/Git control
# state through a generic worker tool, or terminate the Clanker host itself.
# Ordinary command failures, rejected work, and unknown/hallucinated tools do not
# count as an evil condition.
    ...
    try{
        ...
        if($_.Exception.Message -like 'Path escapes worker root:*'){
            Set-SCEvilTrip 'worker.path_escape' "Worker tool '$ToolName' attempted to access a path outside its project root." @{
                taskId=if($Task){[string]$Task.id}else{$null};tool=$ToolName;requestedPath=$Path;workerRoot=(Get-SCRoot)
            }|Out-Null
        }
        throw
    }
...
function Assert-SCWorkerMutablePath([string]$ResolvedPath,$Task,[string]$ToolName) {
    ...
    $protected=($relative.Equals('.statefulclanker',[StringComparison]::OrdinalIgnoreCase) -or
    ...
    if($protected){
        Set-SCEvilTrip 'worker.control_state_tamper' "Worker tool '$ToolName' attempted to mutate orchestration or Git control state directly." @{
            taskId=if($Task){[string]$Task.id}else{$null};tool=$ToolName;requestedPath=$relative;resolvedPath=$ResolvedPath
        }|Out-Null
...
function Test-SCWorkerCommandPathToken([string]$Token) {
    ...
    if($text -match '^--?[^=]+=(.+)function Invoke-SCBoundedCommand([string]$Command,[int]$TimeoutSeconds=120) {
...
function Invoke-SCProvider($Task,[string]$Prompt,[string]$Stage,[string]$ProviderOverride,[string]$ParentAgentId=$null,$Compilation=$null,[string]$WorkerSessionId=$null,[string]$ContinuationMessage=$null) {
    ...
    if($text.StartsWith('~')){return [pscustomobject]@{outside=$true;token=$Token;resolved='<user profile>'}}

    $looksPath=[IO.Path]::IsPathRooted($text) -or $text -match '(^|[\\/])\.\.([\\/]|$)'
    if(-not$looksPath){return $null}

    # Wildcards do not affect the containment decision; trim from the first one
    # before asking Path.GetFullPath to normalize traversal.
    $probe=$text
...
function Assert-SCWorkerCommandSafe([string]$Command,$Task) {
    ...
    foreach($cmd in @($ast.FindAll({param($node) $node -is [System.Management.Automation.Language.CommandAst]},$true))){
        ...
        # allowed; access performed by its ARGUMENTS is what must stay in-project.
        for($i=1;$i-lt$elements.Count;$i++){
            ...
            if($check-and$check.outside){
                Set-SCEvilTrip 'worker.command_path_escape' 'run_command attempted to reference a path outside the project root.' @{
                    taskId=if($Task){[string]$Task.id}else{$null};tool='run_command';token=$check.token;resolvedPath=$check.resolved;command=$Command
                }|Out-Null
    ...
    $mutates='(?i)(Set-Content|Add-Content|Out-File|Remove-Item|Move-Item|Copy-Item|Rename-Item|New-Item|Clear-Content|Set-Item(?:Property)?|Remove-ItemProperty|\[IO\.File\]::(?:Write|Delete|Move|Copy)|(?:^|[;&|])\s*(?:del|erase|rm|rmdir|rd|move|copy)\b|>>?)'
    ...
    if($Command-match$mutates-and$Command-match$control){
        Set-SCEvilTrip 'worker.control_state_tamper' 'run_command attempted to mutate orchestration or Git control state directly.' @{
            taskId=if($Task){[string]$Task.id}else{$null};tool='run_command';command=$Command
        }|Out-Null
