...
function Read-SCCompactPlan([string]$Path) {
    ...
    foreach($raw in $lines){
        ...
        if($key-eq'task'){
            ...
            $task=[ordered]@{id=$value;title='';instruction='';size='small';sources=@();intentRefs=@();acceptance=@();implications=@();proofObligations=@();dependsOn=@();relations=@();retrieval=@();evidence=@();provider=$null;role='worker';humanGate=$false;capabilityProfile=$null;toolAllow=@();toolDeny=@();refinementStatus='pending';refinementDepth=0;parentTaskId=$null;childTaskIds=@();unknown=@()};continue
        }
        if($key-eq'end'){
        ...
        if($null-ne$task){
            switch($key){
              ...
              'source'{$task.sources+=,$value};'intent'{$task.intentRefs+=,$value};'accept'{$task.acceptance+=,$value};'imply'{$task.implications+=,$value};'prove'{$task.proofObligations+=,$value};'depends'{$task.dependsOn+=,$value};'retrieve'{$task.retrieval+=,$value};'evidence'{$task.evidence+=,$value};'provider'{$task.provider=$value};'role'{$task.role=$value}
