...
function Read-SCCompactPlan([string]$Path) {
    ...
    foreach($raw in $lines){
        ...
        if($key-eq'task'){
            ...
            $task=[ordered]@{id=$value;title='';instruction='';size='small';sources=@();intentRefs=@();acceptance=@();dependsOn=@();relations=@();retrieval=@();evidence=@();provider=$null;role='worker';humanGate=$false;capabilityProfile=$null;toolAllow=@();toolDeny=@();unknown=@()};continue
        }
        if($key-eq'end'){
        ...
        if($null-ne$task){
            switch($key){
              ...
              'source'{$task.sources+=,$value};'intent'{$task.intentRefs+=,$value};'accept'{$task.acceptance+=,$value};'depends'{$task.dependsOn+=,$value};'retrieve'{$task.retrieval+=,$value};'evidence'{$task.evidence+=,$value};'provider'{$task.provider=$value};'role'{$task.role=$value}
...
function Import-SCPlan([string]$PlanPath) {
    ...
    foreach($item in @($plan.tasks)){
        ...
        Set-SCProperty $taskObj 'size' $(if($item.PSObject.Properties['size']){[string]$item.size}else{'small'});Set-SCProperty $taskObj 'sources' $(if($item.PSObject.Properties['sources']){@($item.sources)}else{@()});Set-SCProperty $taskObj 'intentRefs' $(if($item.PSObject.Properties['intentRefs']){@($item.intentRefs)}else{@()})
        Set-SCProperty $taskObj 'capabilityProfile' $(if($item.PSObject.Properties['capabilityProfile']-and$item.capabilityProfile){[string]$item.capabilityProfile}else{$null});Set-SCProperty $taskObj 'toolPolicy' $(if($item.PSObject.Properties['toolPolicy']){$item.toolPolicy}else{$null});Save-SCTask $taskObj
    }
...
function Add-SCTask {
    ...
    Set-SCProperty $taskObj 'size' $sizeValue;Set-SCProperty $taskObj 'sources' @($Source);Set-SCProperty $taskObj 'intentRefs' @($IntentRef);Set-SCProperty $taskObj 'capabilityProfile' $CapabilityProfile;Set-SCProperty $taskObj 'toolPolicy' (New-SCTaskToolPolicy $ToolAllow $ToolDeny)
    Save-SCTask $taskObj;Update-SCReadiness;Add-SCEvent 'task.created' $Title @{taskId=$id;size=$sizeValue;capabilityProfile=$CapabilityProfile;toolPolicy=$taskObj.toolPolicy};Write-Host $id
}
