...
- human_required: a genuine human-authority/Intent decision is unresolved; investigate enough to formulate the exact question, then return to the human rather than inventing the answer.
- attention: actively investigate failures, holds, invalidations, accepted/rejected milestones, recovery requests, or intent changes that materially affect progress. Do not merely narrate a recoverable failure.
...
5. Trigger/resume Autofill after a successful repair when appropriate and verify that downstream readiness advances.

Never use recovery to alter or reinterpret current human authority. Never recover-complete a human-gated task, an active task, or a task whose correctness depends on an unresolved design/preference decision. In those cases, stop and ask the human the smallest specific question that blocks further safe repair.
    ...
    switch($action) {
        ...
        'candidate' {
            ...
            try {
                ...
                $goal=Get-McpArgOptional $Arguments 'projectGoal';if($goal){$cli+=@('--goal',$goal)}
...
function Invoke-SCExtendedTool([string]$Name,$Arguments) {
    ...
    switch($Name) {
        ...
        'plan_apply' {
            $planning=Get-McpPlanningSnapshot $project
            if([bool]$planning.active){throw "plan_apply is disabled while isolated planning owns the project (phase: $($planning.phase)). Stage the plan with planning_control candidate/accept instead."}
...
function Invoke-SCSemanticTaskAdd($Arguments) {
    ...
    foreach($pair in @(@('accept','-Accept'),@('check','-Check'),@('judge','-Judge'),@('dependsOn','-DependsOn'),@('retrieval','-Retrieval'),@('evidence','-Evidence'),@('relation','-Relation'),@('source','-Source'),@('intentRef','-IntentRef'))){$values=@(Get-McpArgArray $Arguments $pair[0]);if($values.Count-gt0){$cli+=$pair[1];$cli+=,$values}}
...
function Invoke-McpRpc($Request) {
    ...
    if($method-eq'tools/list') {
        ...
        if($response-and$response.result) {
            ...
            if($task.Count-gt0){$props=$task[0].inputSchema.properties;$props['size']=@{type='string';enum=@('tiny','small','medium','large');description='Semantic size assigned by the conversational planner; never inferred mechanically.'};$props['source']=@{type='array';items=@{type='string'};description='Durable current-source references, normally from current directives.'};$props['intentRef']=@{type='array';items=@{type='string'};description='Governing normalized Intent identifiers.'};$props['check']=@{type='array';items=@{type='string'};description='Executable deterministic acceptance commands. Exit 0 means pass. Prefer these for acceptance whenever correctness is mechanically observable.'};$props['judge']=@{type='array';items=@{type='string'};description='Semantic acceptance criteria that genuinely require inference. Jev is preferred when configured; ordinary validator inference is fallback.'}}
            $plan=@($tools|Where-Object{[string]$_.name-eq'plan_import'}|Select-Object -First 1);if($plan.Count-gt0){$plan[0].description='LEGACY ADDITIVE IMPORT: import a JSON or compact SCPLAN 1 task graph only when isolated planning is not active. Replanning stages a planning_control candidate/handoff.'}
    ...
    if($method-eq'tools/call') {
        ...
        if($name-eq'plan_import'){
            try{
                $project=Get-McpProject $args
                $planning=Get-McpPlanningSnapshot $project
                if([bool]$planning.active){throw "plan_import is disabled while isolated planning owns the project (phase: $($planning.phase)). Stage a planning_control candidate/handoff instead."}
            }catch{return [ordered]@{jsonrpc='2.0';id=$Request.id;result=@{isError=$true;content=@(@{type='text';text=("Tool 'plan_import' failed: {0}"-f$_.Exception.Message)})}}}}
        }
